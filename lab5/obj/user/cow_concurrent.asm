...
obj/__user_cow_concurrent.out:     file format elf64-littleriscv
...
exit(int error_code) {
  ...
  8000d4:	50850513          	addi	a0,a0,1288 # 8005d8 <main+0xb2>
...
printnum(void (*putch)(int, void*), void *putdat,
  ...
  800130:	4c478793          	addi	a5,a5,1220 # 8005f0 <main+0xca>
...
vprintfmt(void (*putch)(int, void*), void *putdat, const char *fmt, va_list ap) {
  ...
  80018c:	49ca8a93          	addi	s5,s5,1180 # 800624 <main+0xfe>
        case 'e':
            err = va_arg(ap, int);
            if (err < 0) {
                err = -err;
  ...
  800194:	6b0b8b93          	addi	s7,s7,1712 # 800840 <error_string>
        while ((ch = *(unsigned char *)fmt ++) != '%') {
  ...
  8003ea:	23a60613          	addi	a2,a2,570 # 800620 <main+0xfa>
  ...
  8003fc:	21860613          	addi	a2,a2,536 # 800610 <main+0xea>
  ...
  80040e:	1fe40413          	addi	s0,s0,510 # 800608 <main+0xe2>
  ...
  800498:	17440413          	addi	s0,s0,372 # 800608 <main+0xe2>
            for (; (ch = *p ++) != '\0' && (precision < 0 || -- precision >= 0); width --) {
  ...
  8004ae:	15fa0a13          	addi	s4,s4,351 # 800609 <main+0xe3>
...
static void fail(const char *m) { cprintf("FAIL: %s\n", m); exit(-1); }
  ...
  800516:	3f650513          	addi	a0,a0,1014 # 800908 <error_string+0xc8>
...
int main(void) {
  ...
  80052c:	3f050513          	addi	a0,a0,1008 # 800918 <error_string+0xd8>
int main(void) {
  ...
  800544:	fb9ff0ef          	jal	ra,8004fc <memset>
    ...
    int pid = fork();
  800548:	b97ff0ef          	jal	ra,8000de <fork>
    if (pid < 0) {
  80054c:	06054963          	bltz	a0,8005be <main+0x98>
        fail("fork failed");
    }

    if (pid == 0) {
  800550:	e51d                	bnez	a0,80057e <main+0x58>
        // child side: ensure parent's area is untouched, then write its own
        if (buf[parent_off] != 'A') fail("child saw parent write");
  800552:	00001717          	auipc	a4,0x1
  800556:	aae74703          	lbu	a4,-1362(a4) # 801000 <buf>
  80055a:	04100793          	li	a5,65
  80055e:	00f70863          	beq	a4,a5,80056e <main+0x48>
  800562:	00000517          	auipc	a0,0x0
  800566:	3e650513          	addi	a0,a0,998 # 800948 <error_string+0x108>
  80056a:	fa5ff0ef          	jal	ra,80050e <fail>
        buf[child_off] = 'C'; // trigger COW on this page
  80056e:	04300793          	li	a5,67
  800572:	00001717          	auipc	a4,0x1
  800576:	28f70723          	sb	a5,654(a4) # 801800 <buf+0x800>
        if (buf[parent_off] != 'A') fail("child saw parent write after COW");
        if (buf[child_off] != 'C') fail("child lost its own write");
        exit(0);
  80057a:	b4fff0ef          	jal	ra,8000c8 <exit>
    }

    // parent side: wait for child, then ensure isolation
    wait();
  80057e:	b63ff0ef          	jal	ra,8000e0 <wait>

    // child write must not affect parent view
    if (buf[child_off] != 'A') fail("parent saw child's write");
  800582:	00001717          	auipc	a4,0x1
  800586:	27e74703          	lbu	a4,638(a4) # 801800 <buf+0x800>
  80058a:	04100793          	li	a5,65
  80058e:	02f71e63          	bne	a4,a5,8005ca <main+0xa4>

    buf[parent_off] = 'P'; // parent write should be private
  800592:	05000793          	li	a5,80
  ...
  80059a:	3ea50513          	addi	a0,a0,1002 # 800980 <error_string+0x140>
    buf[parent_off] = 'P'; // parent write should be private
  80059e:	00001717          	auipc	a4,0x1
  8005a2:	a6f70123          	sb	a5,-1438(a4) # 801000 <buf>
    cprintf("PASS: parent/child writes isolated under COW\n");
  8005a6:	a9bff0ef          	jal	ra,800040 <cprintf>
    cprintf("=== cow_concurrent: done ===\n");
  8005aa:	00000517          	auipc	a0,0x0
  8005ae:	40650513          	addi	a0,a0,1030 # 8009b0 <error_string+0x170>
  ...
  8005c2:	37a50513          	addi	a0,a0,890 # 800938 <error_string+0xf8>
  8005c6:	f49ff0ef          	jal	ra,80050e <fail>
    if (buf[child_off] != 'A') fail("parent saw child's write");
  8005ca:	00000517          	auipc	a0,0x0
  8005ce:	39650513          	addi	a0,a0,918 # 800960 <error_string+0x120>
  8005d2:	f3dff0ef          	jal	ra,80050e <fail>
