               ...
               bool share)
{
    ...
    do
    {
        ...
        if (*ptep & PTE_V)
        {
            ...
            // Extract permissions from the original PTE (excluding PTE_V)
            uint32_t perm = (*ptep & (PTE_R | PTE_W | PTE_X | PTE_U));
            // get page from ptep
            struct Page *page = pte2page(*ptep);
            assert(page != NULL);
            /* LAB5:EXERCISE2 2312819
            ...
            if (share)
            {
                // For shared memory (CLONE_VM), just share the page
                page_ref_inc(page);
                *nptep = *ptep;
            ...
            {
                // COW implementation: Share the page but mark it as read-only with COW flag
                // Only mark as COW if the original page has write permission
                if (perm & PTE_W)
                {
                ...
                {
                    // Page is already read-only, just share it (no COW needed)
                    page_ref_inc(page);
