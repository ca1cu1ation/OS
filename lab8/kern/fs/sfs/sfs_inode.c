  ...
  //LAB8:EXERCISE1 YOUR CODE HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	...
	*/
    ...
    while (nblks != 0) {
        ...
        // 如果buf未对齐，使用sfs_buf_op（内部使用memcpy）以避免对齐问题
        if (((uintptr_t)buf8 % 4) != 0) {
             if ((ret = sfs_buf_op(sfs, buf8, SFS_BLKSIZE, ino, 0)) != 0) {
                goto out;
            }
        } else {
